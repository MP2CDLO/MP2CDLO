...
class ChamferDistanceD1(torch.nn.Module):
    f''' Chamder Distance D1 
         Sigma L2^2 /|X| 
    '''
    def __init__(self, ignore_zeros=False):
        super().__init__()
    ...
    def forward(self, xyz1, xyz2):
        ...
        dist1 = torch.sqrt(dist1)
        dist2 = torch.sqrt(dist2)
        return torch.mean(dist1)*1e3

class ChamferDistanceD1bi(torch.nn.Module):
    f""" Chamder Distance D1bi
    ...
    def forward(self, xyz1, xyz2):
        ...
        dist1 = torch.sqrt(dist1)
        dist2 = torch.sqrt(dist2)
        return (torch.mean(dist1)+torch.mean(dist2))*1e3

class ChamferDistanceD1bi_square(torch.nn.Module):
    f''' Chamder Distance D1bi
         Sigma L2^2 /|X| + Sigma L2^2 /|Y| 
    '''
    def __init__(self, ignore_zeros=False):
